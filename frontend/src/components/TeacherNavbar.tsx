  ...
  return (
    <header className="h-20 border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-40 px-4 lg:px-8 flex items-center justify-between gap-4">
      ...
      <div className="flex items-center gap-6">
        {/* Notifications */}
        <Link to="/teacher/notifications" className="relative p-2.5 bg-gray-50 rounded-xl text-gray-400 hover:text-emerald-500 hover:bg-emerald-50 transition-all group">
        ...
        <button className="flex items-center gap-4 group">
          <div className="text-right hidden sm:block">
            <h4 className="text-sm font-black text-gray-900 group-hover:text-emerald-500 transition-colors">{user?.name || 'Teacher'}</h4>
